...
class SegModel(Model):
    ...
    def __init__(self, channels):
        ...
        self.convU2 = upsample_conv(filter_size[2])
        self.convU3 = upsample_conv(filter_size[3])
        self.convU4 = upsample_conv(filter_size[4])

        #============== deconv layer ==========================================
        
        self.deconv0 = tf.keras.layers.UpSampling2D(size=(2, 2),  interpolation="bilinear")
    ...
    def call(self, inputs):
        ...
        c0 = concatenate([xe_r, xf_r])
        c1 = concatenate([xe1_2_r, xf1_2_r])
        c2 = concatenate([xe2_2_r, xf2_2_r])
        c3 = concatenate([xe3_2_r, xf3_2_r])
        c4 = concatenate([xe4_2_r, xf4_2_r])
        
        '''
        #============= add Event and Frame layers after ResNet Blocks ============================ 
        c0 = self.merge0([xe, xf])               #1/2        
        c1 = self.merge1([xe1_2, xf1_2])         #1/4
        c2 = self.merge2([xe2_2, xf2_2])         #1/8
        c3 = self.merge3([xe3_2, xf3_2])         #1/16
        c4 = self.merge4([xe4_2_r, xf4_2_r])     #1/32
        '''
        ...
        U4 = self.deconv4(C4) # 512 , 1/16 >> 1/8
    ...
    def build_graph(self):
        in1 = Input(shape=(192,336,1) , name = "input1")
        in2 = Input(shape=(192,336,6) , name = "input2") 
        inputs = [in1,in2]
